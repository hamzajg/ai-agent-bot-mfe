...
import "@shared/styles/index.css"; // Tailwind CSS imported
...
function setGlobals(options: ChatBotOptions) {
  ...
  // Product details URL template
  try {
    const tpl = (window as any).__AGENT_CONFIG?.productDetailsUrlTemplate;
    if (tpl) (window as any).__PRODUCT_URL_TEMPLATE = tpl;
  } catch {}
}
...
function init(options: ChatBotOptions = {}) {
  const mount = () => {
    mountWidget(options);
  };
